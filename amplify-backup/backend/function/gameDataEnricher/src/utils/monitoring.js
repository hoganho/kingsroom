// monitoring.js
// Database monitoring utility for gameDataEnricher Lambda
// Adapted from shared lambda-monitoring.js
...
// ===================================================================
// ENRICHMENT-SPECIFIC TRACKING HELPERS
// ===================================================================

/**
 * Track series resolution outcome
 */
const trackSeriesResolution = (monitoring, result) => {
    if (!monitoring) return;
    
    monitoring.trackOperation('SERIES_RESOLUTION', 'TournamentSeries', result.matchedSeriesId, {
...
/**
 * Track recurring game resolution outcome
 */
const trackRecurringResolution = (monitoring, result) => {
    if (!monitoring) return;
    
    monitoring.trackOperation('RECURRING_RESOLUTION', 'RecurringGame', result.matchedRecurringGameId, {
        status: result.status,
        confidence: result.confidence,
        wasCreated: result.wasCreated,
        matchReason: result.matchReason,
...
/**
 * Track venue resolution outcome
 */
const trackVenueResolution = (monitoring, result) => {
    if (!monitoring) return;
    
    monitoring.trackOperation('VENUE_RESOLUTION', 'Venue', result.venueId, {
        status: result.status,
        confidence: result.confidence,
        matchReason: result.matchReason,
        success: !!result.venueId
    });
};
 ...
 */
const trackEnrichmentComplete = (monitoring, result) => {
    if (!monitoring) return;
    
    monitoring.trackOperation('ENRICHMENT_COMPLETE', 'Game', null, {
        success: result.success,
        validationPassed: result.validation?.isValid,
        seriesAssigned: !!result.enrichedGame?.tournamentSeriesId,
        recurringAssigned: !!result.enrichedGame?.recurringGameId,
        venueAssigned: !!result.enrichedGame?.venueId,
        financialsCalculated: result.enrichmentMetadata?.financialsCalculated,
        queryKeysGenerated: result.enrichmentMetadata?.queryKeysGenerated,
        duration: result.enrichmentMetadata?.processingTimeMs
    });
};

module.exports = { 
    LambdaMonitoring,
    trackSeriesResolution,
    trackRecurringResolution,
    trackVenueResolution,
