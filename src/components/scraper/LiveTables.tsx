...
export const LiveTables: React.FC<{ tables?: TableData[] | null }> = ({ tables }) => {
    ...
    return (
        <div className="border rounded-lg bg-white">
            <h4 className="font-bold text-sm text-gray-700 p-3 border-b">Live Tables ({tables?.length || 0})</h4>
            <div className="p-3">
                {hasData ? (
                    <div className="space-y-3">
                     ...
                     <p className="text-xs text-gray-500">No live table data available.</p>
                )}
            </div>
        </div>
    );
