...
/**
 * Convert ISO datetime to date-only string for input[type="date"]
 * "2023-02-25T19:00:00.000Z" -> "2023-02-25"
 */
const toDateInputValue = (isoDateTime: string | null | undefined): string => {
...
/**
 * Convert date-only string to ISO datetime for storage
 * "2023-02-25" -> "2023-02-25T00:00:00.000Z"
 */
const toISODateTime = (dateString: string | null | undefined): string | null => {
...
export const SeriesInstanceManager: React.FC<Props> = ({ seriesInstances, seriesTitles, venues, onSave, onDelete }) => {
    ...
    useEffect(() => {
        if (formState.startDate) {
            const dateString = toDateInputValue(formState.startDate);
            if (dateString) {
                const { year, quarter, month } = getDateComponents(dateString);
                setFormState(prev => ({
                    ...prev,
                    year: year,
                    quarter: quarter,
    ...
    const handleSave = () => {
        ...
        const saveData = {
            ...formState,
            startDate: toISODateTime(formState.startDate),
            endDate: toISODateTime(formState.endDate)
        };
        onSave(saveData);
        setIsFormOpen(false);
        setFormState({});
                ...
                <div className="p-4 border rounded-md bg-gray-50 space-y-3">
                        ...
                        {formState.seriesCategory === APITypes.SeriesCategory.SPECIAL && (
                            <div>
                                ...
                                <select 
                                    ...
                                    onChange={e => setFormState({...formState, holidayType: e.target.value as APITypes.HolidayType})} 
                                    className="w-full p-2 border border-gray-300 rounded focus:border-indigo-500 focus:outline-none"
                                >
                                    ...
                                    <option value={APITypes.HolidayType.MELBOURNE_CUP}>Melbourne Cup</option>
                                    <option value={APITypes.HolidayType.QUEENS_BIRTHDAY}>Queen's Birthday</option>
                                    <option value={APITypes.HolidayType.LABOUR_DAY}>Labour Day</option>
                    ...
                    <div className="flex space-x-2 pt-3 border-t">
                        ...
                        <div className="flex-1">
                            ...
                            <input 
                                type="date" 
                                value={toDateInputValue(formState.endDate)} 
            ...
            <div className="space-y-2">
                {seriesInstances.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                        No series instances found. Create one above.
                    </div>
                ) : (
                        ...
                        <div key={instance.id} className="flex justify-between items-center p-3 border rounded-md hover:bg-gray-50">
                            <div>
                                ...
                                <div className="flex items-center space-x-2 mt-1">
                                    ...
                                    {instance.numberOfEvents !== null && instance.numberOfEvents !== undefined && (
                                        <span className="text-xs text-gray-500">
                                            {instance.numberOfEvents} events
                                        </span>
                                    )}
                                </div>
                                {/* Date range display */}
                                {(instance.startDate || instance.endDate) && (
                                    <div className="text-xs text-gray-500 mt-1">
                                        {formatDateDisplay(instance.startDate)} - {formatDateDisplay(instance.endDate)}
                                    </div>
                                )}
                            </div>
